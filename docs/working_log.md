...
| `GET/POST /api/jobs/{id}/calibration` | ✅ | ☑ | 解析 solve-field WCS（astropy） |
| `GET/POST /api/jobs/{id}/tags` | ✅ | ☑ | 初期返回空 |
| `GET/POST /api/jobs/{id}/machine_tags` | ✅ | ☑ | ditto |
| `GET/POST /api/jobs/{id}/objects_in_field` | ✅ | ☑ | 来自 solve-field “Your field contains” 列表 |
...
1. Annotated image 仍为 placeholder PNG（仅含文本），需要决定是否接入 astrometry 绘图脚本或自研叠加方案。
